class MailToGuestWorker
  ...
  def perform event_id
    ...
    if event.activate? && event.room_active == "opened"
      GuestMailer.delay_for(30.seconds).invitation_guest(event.user, event)
      users.each do |user|
        GuestMailer.delay_for(30.seconds).invitation_guest(user, event)
      end
    elsif event.inactivate? && event.room_active == "locked"
      users.each do |user|
        GuestMailer.delay_for(30.seconds).cancel_invitation(user, event)
      end
    end
  end
end
